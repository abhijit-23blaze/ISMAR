...
async function loadData() {
    try {
        ...
        if (Array.isArray(parsedFrames)) {
            ...
            throw new Error("current_frames.json is not a valid JSON array.");
        }
    }
    catch (error) {
...
function calculatePoseDeviation(playerFrame, idealFrame) {
    if (!playerFrame.relative_distance_matrix || !idealFrame.relative_distance_matrix || playerFrame.available_joints.length === 0) {
        return { accuracy: 0, feedback: ["Not enough data to analyze pose."], mainDeviations: [] };
    }
    const playerMatrix = playerFrame.relative_distance_matrix;
    const idealMatrix = idealFrame.relative_distance_matrix;
    const playerJoints = new Set(playerFrame.available_joints);
    const idealJoints = new Set(idealFrame.available_joints);
    let totalDeviation = 0;
    let comparisons = 0;
    const deviations = [];
    for (const j1 of playerFrame.available_joints) {
        if (!idealJoints.has(j1))
            continue;
        for (const j2 of playerFrame.available_joints) {
            if (j1 === j2)
                continue;
            if (!idealJoints.has(j2))
                continue;
            const key1 = j1 < j2 ? j1 : j2;
            const key2 = j1 < j2 ? j2 : j1;
            const playerDist = playerMatrix[key1]?.[key2] ?? playerMatrix[key2]?.[key1];
            const idealDist = idealMatrix[key1]?.[key2] ?? idealMatrix[key2]?.[key1];
            if (playerDist !== undefined && idealDist !== undefined && idealDist > 0) {
                const diff = Math.abs(playerDist - idealDist);
                const deviationPercentage = (diff / idealDist) * 100;
                totalDeviation += deviationPercentage;
                deviations.push({ pair: `${key1}-${key2}`, diff: deviationPercentage });
                comparisons++;
            }
        }
    }
    if (comparisons === 0) {
        return { accuracy: 0, feedback: ["Could not compare any joint pairs."], mainDeviations: [] };
    }
    const averageDeviation = totalDeviation / comparisons;
    const accuracy = Math.max(0, Math.min(100, 100 - averageDeviation));
    deviations.sort((a, b) => b.diff - a.diff);
    const topDeviations = deviations.slice(0, 3);
    const feedback = [];
    const mainDeviationDescriptions = [];
    if (accuracy < 70)
        feedback.push("Focus on the overall form and alignment.");
    else if (accuracy < 90)
        feedback.push("Good alignment! Minor adjustments needed.");
    else
        feedback.push("Excellent form! Maintain this stability.");
    ...
    if (Math.abs(playerFrame.player_height - idealFrame.player_height) > 0.1) {
        feedback.push(`Note: Your height (${playerFrame.player_height.toFixed(2)}m) differs from the ideal model (${idealFrame.player_height.toFixed(2)}m), which may slightly affect comparison.`);
    }
...
server.tool("read-frame", "Read a specific yoga pose frame", {
    ...
    try {
        ...
        const frameTimestamp = isNaN(Number(timestampStr)) ? timestampStr : Number(timestampStr);
...
server.tool("generate-session-report", "Generates a comprehensive report analyzing the entire yoga session based on stored frame data.", {}, async (_params) => {
    try {
        ...
        const poseStats = {};
        const allPerformedPoses = new Set();
        ...
        for (let i = 0; i < loadedFrames.length; i++) {
            ...
            if (gameState && gameState !== "Relaxed") {
                ...
                const poseInfo = medicalInfo.poses.find((p) => p.name === gameState);
                if (poseInfo) {
                    (poseInfo.benefits || []).forEach((b) => allBenefits.add(b));
                ...
                const poseInfo = medicalInfo.poses.find((p) => p.name === "Relaxed");
